...
def train(data_train,data_val,train_mask,val_mask,
    ...
    train_np = train_np[:30208].reshape(-1*(seq_length*2),data_train.shape[1]) #original 30,711, para 32=30688
    data_train = pd.DataFrame(train_np, columns = data_train.columns)
    
    val_np = data_val.to_numpy().reshape(-1,(seq_length*2)*data_val.shape[1]) #original 7703, para 32=7680
    ...
    train_loader = DataLoader(train_data,batch_size=batch_size,num_workers=6,shuffle=True) #num_workers=6
    val_loader = DataLoader(val_data,batch_size=batch_size,num_workers=5,shuffle=False) #num_workers=5
...
if __name__ == "__main__":
    ...
    # lr_finder = trainer.tuner.lr_find(model,train_loader, val_loader,max_lr=1.0,min_lr=1e-6)
    # print("LR suggestion:",lr_finder.suggestion())
    # fig = lr_finder.plot(show=True,suggest=True) #plotting LR finder
