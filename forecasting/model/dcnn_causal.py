"""
Dilated causal convolutional neural networks for predicting multivariate time series
...
class rnn(torch.nn.Module):
    ...
    def forward(self, x_input, cond): 
        """
        Parameters:
        input = ([batch_size,input_seqlength,num_features])
        cond = ([batch_size,num_conditions]) 
        Return:
        output = [batch_size,input_seqlength,100]
        """
        cond = cond.reshape(2,-1,cond.shape[1])
        h0 = self.dense(cond)
        h0 = h0.reshape(2,x_input.shape[0],-1)
        output, _ = self.rnn(x_input, (h0,h0))
        return output
...
class DC_CNN_Block(torch.nn.Module):
    ...
    def forward(self, x_input,cond):
        ...
        skip_out = skip_out.permute(0,2,1).to(dev)
        network_in = self.network_in(layer_out)
        network_in = network_in.permute(0,2,1).to(dev)
...
class CausalModel(pl.LightningModule):
    ...
    def forward(self,x_input,cond): 
        ...
        output sequence: [batch_size, input_seq_length,num_features]
        """
    ...
    def training_step(self, batch, batch_idx):
        ...
        dilate,loss_shape,loss_temporal = dilate_loss(ypred,target_out,batch_size=self.batch_size,
                                                     ...
                                                     mask = "False")
        self.log("loss",dilate, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        return {"loss":dilate,"past":target_in,"ytrue":target_out,"ypred":ypred,"conditions":condition, "mask":mask}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_loss",dilate)
        return {"val_loss":dilate,
    ...
    def test_step(self, batch, batch_idx):
        ...
        dilate,loss_shape,loss_temporal = dilate_loss(ypred_mask,target_mask,batch_size=self.batch_size,
                                                      ...
                                                      alpha=self.alpha,gamma=self.gamma,device=self.device,
                                                      mask = "True")
        self.log("test_loss",dilate)
        return {"test_loss":dilate}
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW([
                ...
                lr=5e-5, weight_decay=1e-8,betas=(0.9, 0.999))
