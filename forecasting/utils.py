...
def plotting_predictions_comparative(real_input,real_target,pred,seq_length,
                         num_features,feature_list,path, net, conditions,
                         num_conditions, mask,flag="val", epoch="none",num_samples=2):

    print("* Opening y_pred (RCGAN)")
    pred_rcgan=pd.read_csv("/home/blanca/anaconda3/envs/time_series_forecasting/experimentos/rcgan/data/ypred.csv")
    pred_rcgan=pred_rcgan[feature_list]

    print("* Opening y_pred (TIMEGAN)")
    pred_timegan=pd.read_csv("/home/blanca/anaconda3/envs/time_series_forecasting/experimentos/timegan/ypred.csv")
    pred_timegan=pred_timegan[feature_list]

    real_input = np.array(real_input).reshape(-1,seq_length,num_features)#(128, 6, 8)
    real_target = np.array(real_target).reshape(-1,seq_length,num_features)#(128, 6, 8)
    pred = np.array(pred).reshape(-1,seq_length,num_features)#(128, 6, 8)
    mask = np.array(mask).reshape(-1,seq_length,num_features)#(128, 6, 8)
    ...
    pred_rcgan = pred_rcgan.to_numpy().reshape(-1,seq_length,num_features)
    pred_timegan = pred_timegan.to_numpy().reshape(-1,seq_length,num_features)

    # #selección de índices aleatorios from pred_seqs
    np.random.seed(242)
    indices = np.random.choice(range(real_input.shape[0]),replace=False,size=num_samples)
    ...
    for index in indices: #recorriendo los indices
        ...
        prediction_rcgan = pred_rcgan[index,:,:] #(24, 8)
        prediction_timegan = pred_timegan[index,:,:] #(24, 8)
        ...
        if num_conditions > 0:
            ...
            sex_pd['Sex'] = np.where(sex_pd['Sex'] == 1, 'Man','Women')
            #-------------#

        pd_real_values  = pd.DataFrame(data_array,columns= feature_list)
        ...
        pd_rcgan = pd.DataFrame(prediction_rcgan,columns= feature_list)
        pd_timegan = pd.DataFrame(prediction_timegan,columns= feature_list)
        ...
        seq_length = pd_past_pat.shape[0]
        row, col  = 0,0

        for feature in feature_list:            
            ax[row,col].plot(range(seq_length),pd_past_pat[feature], "r.-", label="History", color = "blue")

            ax[row,col].plot(range(seq_length-1, seq_length + seq_length),np.concatenate([pd_past_pat[feature]
                        [seq_length-1:seq_length], pd_expected_pat[feature]]),
                        "r.-",label="Ground truth",color = "orange")
            
            ax[row,col].plot(range(seq_length-1, seq_length + seq_length),np.concatenate([pd_past_pat[feature]
                        [seq_length-1:seq_length], pd_prediction_pat[feature]]), 
                        "r.-", label="Prediction (our)", color="green")

            ax[row,col].plot(range(seq_length-1, seq_length + seq_length),np.concatenate([pd_past_pat[feature]
                        [seq_length-1:seq_length], pd_rcgan[feature]]), 
                        "r.-", label="Prediction (rcgan)", color="purple")

            ax[row,col].plot(range(seq_length-1, seq_length + seq_length),np.concatenate([pd_past_pat[feature]
                        [seq_length-1:seq_length], pd_timegan[feature]]), 
                        "r.-", label="Prediction (timegan)", color="red")

            ax[row,col].plot(range(seq_length-1, seq_length + seq_length),np.concatenate([pd_past_pat[feature]
                        [seq_length-1:seq_length], pd_real_values[feature]]), 
                        marker='o',markersize=10,label="Real_values",color = "orange")
            ax[row,col].set_title(feature)
            ax[row,col].grid(True,color='lightgrey',alpha=0.2)
            ax[row,col].set_xlabel("Hours")
            ax[row,col].set_ylabel("Values")

            if row == 3: #cambio de columna
                if num_conditions == 5:
                    ...
                    ax[row,col].legend(title='Sex: '+str(sex_pd['Sex'].item())+' Age:'+str(age)
                                       + ' Diabetes:'+str(dm) +' Hypertension:'+str(hyp)+' Heart diseases:'+str(hd),
                                       loc='upper center', bbox_to_anchor=(0.5, -0.25),fancybox=True, shadow=True, ncol=5)
                elif num_conditions == 10:
                    dm = conditions_per_patient[2].astype(int)
                    hyp = conditions_per_patient[3].astype(int)
                    hd = conditions_per_patient[4].astype(int)

                    tsicu = conditions_per_patient[5].astype(int)
                    sicu = conditions_per_patient[6].astype(int)
                    micu = conditions_per_patient[7].astype(int)
                    ccu = conditions_per_patient[8].astype(int)
                    csru = conditions_per_patient[9].astype(int)
                    

                    ax[row,col].legend(title='Sex: '+str(sex_pd['Sex'].item())+' Age:'+str(age)
                                       + ' Diabetes:'+str(dm) +' Hypertension:'+str(hyp)+' Heart diseases:'+str(hd)
                                       + ' TSICU:'+str(tsicu) + ' SICU:'+str(sicu) + ' MICU:'+str(micu)
                                       + ' MICU:'+str(micu) + ' CCU:'+str(ccu) + ' CSRU:'+str(csru)
                                       ,loc='upper center', bbox_to_anchor=(0.5, -0.25),fancybox=True, shadow=True, ncol=5)
                    ...
                    ax[row,col].legend(loc='upper center', bbox_to_anchor=(0.5, -0.25),fancybox=True, shadow=True, ncol=5)
                col = 1
                row = 0
            else:
                row+=1
        plt.savefig(path+'plots/'+str(flag)+"_epoch_"+str(epoch)+"_pat"+str(index)+".png")
        plt.close() 
...
def comparative(real_input,real_target,pred,mask,input_seq_length,output_seq_length,
                         num_features,num_conditions,conditions,path, net, num_samples,mode,fold):
    feature_list = ['arterial_bp_mean','respiratory_rate', 'diastolic_bp','spo2','heart_rate', 'systolic_bp', 'temperature']
    
    print("* Opening y_pred (RCGAN)")
    pred_rcgan=pd.read_csv("/home/blanca/anaconda3/envs/synthm/RGAN/experimentos/CGAN_condicionada/nstemi/data/ypred_nstemi.csv")
    pred_rcgan=pred_rcgan[feature_list]

    print("* Opening y_pred (TIMEGAN)")
    pred_timegan=pd.read_csv("/home/blanca/anaconda3/envs/tgan/TimeGAN/resultados_nstemi/ypred_nstemi.csv")
    pred_timegan=pred_timegan[feature_list]

    print("* Opening y_pred (RMSE)")
    pred_rmse=pd.read_csv("/home/blanca/anaconda3/envs/time_series_forecasting/experimentos/causal_in6_out6_c5_rmse/data/ypred_nstemi.csv")
    pred_rmse=pred_rmse[feature_list]
    ...
    pred_rcgan = pred_rcgan.to_numpy().reshape(-1,output_seq_length,num_features)
    pred_timegan = pred_timegan.to_numpy().reshape(-1,output_seq_length,num_features)
    pred_rmse = pred_rmse.to_numpy().reshape(-1,output_seq_length,num_features)
    ...
    for index in indices: #recorriendo los indices
        ...
        prediction_rcgan = pred_rcgan[index,:,:] #(24, 8)
        prediction_timegan = pred_timegan[index,:,:] #(24, 8)
        prediction_rmse = pred_rmse[index,:,:] #(24, 8)

        pd_past_pat = pd.DataFrame(past_pat,columns= feature_list)
        pd_expected_pat = pd.DataFrame(expected_pat,columns= feature_list)
        pd_prediction_pat = pd.DataFrame(prediction_pat,columns= feature_list)
        pd_rcgan_pat = pd.DataFrame(prediction_rcgan,columns= feature_list)
        pd_timegan_pat = pd.DataFrame(prediction_timegan,columns= feature_list)
        pd_rmse_pat = pd.DataFrame(prediction_rmse,columns= feature_list)
        ...
        pd_rcgan_pat = rename_columns(pd_rcgan_pat)
        pd_timegan_pat = rename_columns(pd_timegan_pat)
        pd_rmse_pat = rename_columns(pd_rmse_pat)
        features_trans = pd_prediction_pat.columns
        ...
        for i in range(7):
            ...
            ax.plot(range(pd_past_pat.shape[0]),pd_past_pat[feature],marker='o',markersize=10,label="Observaciones", color = "darkblue")

            ax.plot(range(pd_past_pat.shape[0]-1, pd_past_pat.shape[0]+pd_expected_pat.shape[0]),
                    np.concatenate([pd_past_pat[feature][pd_past_pat.shape[0]-1:pd_past_pat.shape[0]], pd_expected_pat[feature]]),
                    "r.-",label="Valor imputado",color = "orange")
        
            ax.plot(range(pd_past_pat.shape[0]-1, pd_past_pat.shape[0]+pd_expected_pat.shape[0])
                        ,np.concatenate([pd_past_pat[feature]
                        [pd_past_pat.shape[0]-1:pd_past_pat.shape[0]], pd_prediction_pat[feature]]),
                        'o--', label="Predicción (Dilate)",color='green',markersize=10,)

            ax.plot(range(pd_past_pat.shape[0]-1, pd_past_pat.shape[0]+pd_expected_pat.shape[0])
                        ,np.concatenate([pd_past_pat[feature]
                        [pd_past_pat.shape[0]-1:pd_past_pat.shape[0]], pd_rcgan_pat[feature]]),
                        'o--', label="Predicción (RCGAN)",color='purple',markersize=10,)

            # ax.plot(range(pd_past_pat.shape[0]-1, pd_past_pat.shape[0]+pd_expected_pat.shape[0])
            #             ,np.concatenate([pd_past_pat[feature]
            #             [pd_past_pat.shape[0]-1:pd_past_pat.shape[0]], pd_timegan_pat[feature]]),
            #             'o--', label="Predicción (TIMEGAN)",color='black',markersize=10,)

            ax.plot(range(pd_past_pat.shape[0]-1, pd_past_pat.shape[0]+pd_expected_pat.shape[0])
                        ,np.concatenate([pd_past_pat[feature]
                        [pd_past_pat.shape[0]-1:pd_past_pat.shape[0]], pd_rmse_pat[feature]]),
                        'o--', label="Predicción (RMSE)",color='red',markersize=10,)
        ...
        if num_conditions == 2:
            ax.legend(title='Sex: '+str(sex_pd['Sex'].item())+' Age:'+str(age),
                      loc='upper center', bbox_to_anchor=(0.5, -0.35),fancybox=True, shadow=True, ncol=5)
        ...
        plt.savefig(path+'plots/'+"comparative_pat_"+str(index)+".png")
        plt.close()
...
def computing_rmse_stateOfArt(ytrue,ypred,output_seq_length,feature,mode,path): #ytrue: (100, 6, 7) ypred: (100, 6, 7)
    import math
    from sklearn.metrics import mean_squared_error
    list_arterial,list_resp,list_dias,list_spo2,list_heart,list_sys,list_temp=[],[],[],[],[],[],[]
    feature_list = ['arterial_bp_mean','respiratory_rate', 'diastolic_bp','spo2','heart_rate', 'systolic_bp', 'temperature']
    ...
    # print("* Opening y_pred (RGAN)")
    # pred_=pd.read_csv("/home/blanca/anaconda3/envs/synthm/RGAN/experimentos/RGAN_nstemi/data/ypred.csv")
    # pred_=pred_[feature_list]

    # print("* Opening y_pred (RCGAN)")
    # pred_=pd.read_csv("/home/blanca/anaconda3/envs/synthm/RGAN/experimentos/CGAN_condicionada/nstemi/data/ypred.csv")
    # pred_=pred_[feature_list]

    print("* Opening y_pred (TIMEGAN)")
    pred_=pd.read_csv("/home/blanca/anaconda3/envs/tgan/TimeGAN/resultados_nstemi/ypred.csv")
    pred_=pred_[feature_list]


    ytrue = np.array(ytrue).reshape(-1,output_seq_length,len(feature))#(128, 6, 8)
    ypred = np.array(pred_).reshape(-1,output_seq_length,len(feature))#(128, 6, 8)

    print("ytrue:",ytrue.shape,"ypred:",ypred.shape)

    ypred = ypred[:ytrue.shape[0]]
    if mode =="escalado":
        ytrue = rescale_data_test(ytrue,path)#[32, 24, 8])
        ytrue = np.array(ytrue).reshape(-1,output_seq_length,len(feature))#(128, 6, 8)
    ...
    dict ={"arterial_bp_mean":str(round(np.mean(list_arterial),2)),
           "respiratory_rate":str(round(np.mean(list_resp),2)),
           "diastolic_bp":str(round(np.mean(list_dias),2)),
           "spo2":str(round(np.mean(list_spo2),2)),
           "heart_rate":str(round(np.mean(list_heart),2)),
           "systolic_bp":str(round(np.mean(list_sys),2)),
           "temperature":str(round(np.mean(list_temp),2)),}

    pathfile = path + "metrics/TIMEGAN_NSTEMI_rmse_per_feature_"+str(mode)+".txt"
    with open(pathfile, 'w') as f:
        print(dict, file=f)
