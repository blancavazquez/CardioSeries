...
def split_df(df,in_seqlength,out_seqlength):
    ...
    history = df[start_index:in_seqlength] #first 24 hours
    targets = df[in_seqlength:end_index] #next 24 hours
...
class KFoldDataModule(BaseKFoldDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        seq_length = 6
        train_np = self.data_train.to_numpy().reshape(-1,(seq_length*2)*self.data_train.shape[1])
        train_np = train_np[:30208].reshape(-1*(seq_length*2),self.data_train.shape[1])
        data_train = pd.DataFrame(train_np, columns = self.data_train.columns)
        
        val_np = self.data_val.to_numpy().reshape(-1,(seq_length*2)*self.data_val.shape[1])
