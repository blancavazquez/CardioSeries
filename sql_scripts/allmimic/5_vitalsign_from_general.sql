...
select
  ...
  ,nt."charttime"
  ,dose,unit
  ,REPLACE(REPLACE(REPLACE(REPLACE(nt.dose,',',''),'[**2133-12-29**]','0'),'[**2153-1-25**]','0'),'[**2147-3-23**]','') as value
 ,case when LOWER(nt."unit") = 'units/hour' then 'unit' else null end as valueuom
...
select
  ...
  ,nt."charttime" as "CHARTTIME"
  ,value
  --,case when nt."value"::double precision > 0 then 217 else null end as "ITEMID"
  --,case when (nt."value"::>0 and char_length(nt."value")>0) then nt."value"::double precision else 0 end as "VALUE"
  ,nt."valueuom" as "VALUEUOM"
