...
--***********************************************************************************
--Regex 1: troponin (physician)
drop view physician_troponin_temp1 cascade;
create view physician_troponin_temp1 as
...
select nt.subject_id
      ,nt.hadm_id
      ,nt.chartdate
      ,regexp_matches(LOWER(texto_final), 'troponin-t:[0-9:]+/[0-9:]+/([0-9:].[0-9:]+)', 'g') as tropo
      ,substring(texto_final, 'Flowsheet Data as of ([\[\]0-9*-]+)') as fecha
...
select nt.subject_id
      ,nt.hadm_id
      ,nt.chartdate
      ,REPLACE(REPLACE(REPLACE(tropo::text,'{',''),'}',''),',','')::double precision as value
...
--***************************************************************
--Regex 2: Troponin (physycian)
drop view physician_troponin_temp2 cascade;
create view physician_troponin_temp2 as
with temp_note_sin_saltos as
(
select nt.subject_id
      ,nt.hadm_id
...
select nt.subject_id
      ...
      ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
              REPLACE(REPLACE(
              REPLACE(REPLACE(tropo::text,'{',''),'}',''),':',''),'" 0"','0'),'/',''),',',''),'"',''),'<',''),'h',''),'-','') as value
      ,REPLACE(REPLACE(fecha::text,'{"[**',''),'**]','') as fecha2  
      ,hora 
...
select nt.subject_id as "SUBJECT_ID"
      ,nt.hadm_id  as "HADM_ID"
      ,nt.chartdate as "CHARTDATE"
      ,case when (char_length(fecha2)>7) then concat(fecha2,' ',hora)::timestamp else nt.chartdate end as "CHARTTIME"
      ,case when (value::double precision>0) then 227429 else null end as "ITEMID"
      ,value::double precision as "VALUE"
      --,value
...
drop view physician_ckmb_temp1 cascade; ---ok
create view physician_ckmb_temp1 as
with temp_note_sin_saltos as
(
select nt.subject_id
      ,nt.hadm_id
...
--*******************************************************************************
--Regex 1 : ck (physician)
drop view physician_ck_temp1 cascade;
create view physician_ck_temp1 as
with temp_note_sin_saltos as
(
select nt.subject_id
      ,nt.hadm_id
...
select nt.subject_id
      ...
      ,nt.chartdate
      ,ck
      ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(ck::text,'{',''),'}',''),':',''),'"',''),'/',''),' ',''),',','')as value--::double precision as value
      ,REPLACE(REPLACE(fecha::text,'{"[**',''),'**]','') as fecha2  
      ,hora 
...
--****************************************
--Regex 1 : mb (physician)
drop view physician_mb_temp1 cascade;
create view physician_mb_temp1 as
with temp_note_sin_saltos as
(
select nt.subject_id
      ,nt.hadm_id
...
select nt.subject_id
      ...
      ,nt.chartdate
      ,mb0,mb
      ,REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(mb::text,'{',''),'}',''),':',''),'"',''),'/',''),' ',''),',',''),'>','')as value--::double precision as value
      ,REPLACE(REPLACE(fecha::text,'{"[**',''),'**]','') as fecha2  
      ,hora 
...
drop view biomarcadores_physician_stemi cascade;
create view biomarcadores_physician_stemi as
...
,"VALUE"
,"VALUEUOM"
from physician_troponin_temp1 --ok
UNION
select
"SUBJECT_ID"
,"HADM_ID"
...
,"VALUE"
,"VALUEUOM"
from physician_troponin_temp2--ok
UNION
select
"SUBJECT_ID"
,"HADM_ID"
...
,"VALUE"
,"VALUEUOM"
from physician_mb_temp1 --ok
order by "SUBJECT_ID";

--select * from biomarcadores_physician_stemi;
