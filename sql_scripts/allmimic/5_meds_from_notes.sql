...
drop view all_lisinopril cascade;

drop view all_enalapril cascade;
drop view all_captopril cascade;
drop view all_atorvastatin cascade;
drop view all_simvastatin cascade;
drop view all_pravastatin cascade;
drop view all_rosuvastatin cascade;
drop view all_metoprolol cascade;
drop view all_atenolol cascade;
drop view all_carvedilol cascade;
drop view all_sotalol cascade;
drop view all_aspirin cascade;
drop view consult_data cascade;
drop view echo_data cascade;
---------------------------

--Paso 1: extracción de lisinopril
drop view all_lisinopril cascade;
create view all_lisinopril as
...
--**************************************
--Paso4: extracción de atorvastatin
drop view all_atorvastatin cascade;
create view all_atorvastatin as
with temp_extract_dosis as
(
select
  nt.subject_id
...
select
  ...
  ,nt."minutes"
  ,nt."fecha"
  ,REPLACE(nt."hours"::text,':','') as hours2
  ,REPLACE(nt."minutes"::text,':','') as minutes2
...
select
  ...
  ,nt.chartdate
  ,nt."fecha"
  ,concat(nt."fecha", ' ', nt."hours2",':',nt."minutes2",':00') as date_hour
  ,nt."value"
...
select
  ...
  ,nt.hadm_id as "HADM_ID"
  ,nt.chartdate as "CHARTDATE"
  ,case when CHAR_LENGTH(nt."date_hour") = '15' then nt.chartdate
            else nt.date_hour::timestamp end as "CHARTTIME"

  ,case when nt."value"!='' then 286651 else null end as "ITEMID"
  ,nt."value"::double precision as "VALUE"

  ,case when nt."value"!='' then 'mg' else null end as "VALUEUOM"

FROM temp_longitud nt
where "value"::double precision>0 and "value"!='.'
order by nt.subject_id;

--*************************************************
--Paso5: extracción de simvastatin
drop view all_simvastatin cascade;
create view all_simvastatin as
with temp_extract_dosis as
(
select
  nt.subject_id
...
drop view all_pravastatin cascade;
create view all_pravastatin as
with temp_extract_dosis as
(
select
  nt.subject_id
...
drop view all_rosuvastatin cascade;
create view all_rosuvastatin as
with temp_extract_dosis as
(
select
  nt.subject_id
...
select
  ...
  ,nt.hadm_id
  ,nt.chartdate
  ,REPLACE(nt."value"::text,':','')::double precision as value2
  ---,cast(nt."value" as double precision)
  ,nt."hours"
  ,nt."minutes"
  ,concat(nt."fecha", ' ', nt."hours",':',nt."minutes",':00') as date_hour
...
select
  ...
  ,nt.date_hour
  ,CHAR_LENGTH(nt."date_hour") as longitud
  ,case when nt."value2"::double precision>1000 then nt."value2"::double precision/1000
            else nt."value2" end as value3 -- corrección de valores atípicos!
...
select
  ...
  ,case
    when nt."value3">0 then 42647
  else null end as "ITEMID"

  ,nt."value3"::double precision as "VALUE"

  ,case
    when nt."value3">0 then 'mg'
  else null end as "VALUEUOM"
...
FROM temp_longitud nt
where "value2" is not null ---and "value"!='.'
order by nt.subject_id;
...
--\copy (SELECT * FROM all_aspirin) to '/tmp/all_aspirin.csv' CSV HEADER;
--select * from all_aspirin order by "VALUE" desc;
...
drop view consult_data;
create view consult_data as--extract medications from consultant notes
...
drop view all_medicaments cascade;
create view all_medicaments as
select
"SUBJECT_ID"
,"HADM_ID"
,"CHARTDATE"
...
,"VALUE"::double precision
,"VALUEUOM"
from consult_data --amiodarone data from category:consult ok
where "VALUE">0
order by "SUBJECT_ID";

--select * from all_medicaments;
