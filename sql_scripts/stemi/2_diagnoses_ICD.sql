...
-- procedures_icd for STEMI patients
...
drop view STEMI_procedures_icd;
create view STEMI_procedures_icd as
with temp_item as
(
select
   proc.subject_id
...
--**************************************************
-- Paso 3: Script for joining diagnoses + procedures for STEMI patients

drop view STEMI_dx_procedures;
create view STEMI_dx_procedures as
...
select 
  ...
  ,"final_itemid"
  ,seq_num
from STEMI_procedures_icd
order by subject_id;

\copy (SELECT * FROM STEMI_dx_procedures) to '/tmp/DIAGNOSES_ICD.csv' CSV HEADER;
