...
drop view STEMI_prescriptions_preprocess cascade;
create view STEMI_prescriptions_preprocess as
  select
  pre.subject_id
  ,pre.hadm_id
  ,pre.icustay_id
...
drop view STEMI_prescriptions_join;
create view STEMI_prescriptions_join as
  select
  pre.subject_id
  ,pre.hadm_id
  ,pre.icustay_id
  ...
  from STEMI_prescriptions_preprocess pre
  order by pre.subject_id;

-- ******************************************************************
-- Paso 3: identificar tipo de unidad de medida por medicamento de interés
drop view STEMI_prescriptions_unit;
create view STEMI_prescriptions_unit as
  select
  pre.subject_id
  ,pre.drug
  ,pre.final_dosis
  ...
  from STEMI_prescriptions_join pre
...
drop view STEMI_prescriptions_itemid_dosis;
create view STEMI_prescriptions_itemid_dosis as
...
drop view STEMI_prescriptions_double;
create view STEMI_prescriptions_double as
  select
  ...
  from STEMI_prescriptions_itemid_dosis pre
  order by pre.subject_id;
\copy (SELECT * FROM STEMI_prescriptions_double) to '/tmp/PRESCRIPTIONS.csv' CSV HEADER;
