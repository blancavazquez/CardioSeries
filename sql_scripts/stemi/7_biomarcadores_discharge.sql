...
select nt.subject_id
      ...
      ,nt.chartdate::timestamp
      ,ck
      ,REPLACE(ck_clean,'ck-','ck ') as ck_clean2
...
select nt.subject_id
      ...
      ,nt.chartdate
      ,ck
      ,split_part(nt.ck_clean2,' ',1) as fecha
      ,split_part(nt.ck_clean2,' ',2) as hora
      ,split_part(nt.ck_clean2,' ',4) as value_previo     
...
select nt.subject_id
      ...
      ,nt.chartdate
      ,ck
      ,fecha,hora,value_previo
      ,REPLACE(hora,'ck','') as hora2
      ,case when (char_length(hora)>=5) then value_previo else split_part(nt.ck::text,' ',3) end as value2
FROM temp_seleccion_value_empty nt
order by nt.subject_id
),
temp_concat_fecha_hora as (
select nt.subject_id
      ...
      ,ck
      ,value2
      ,REPLACE(value2,'"}','')::double precision as value
      ,fecha,hora,hora2
...
select nt.subject_id as "SUBJECT_ID"
      ,nt.hadm_id as "HADM_ID"
      ,nt.chartdate as "CHARTDATE"
      ,case when ((value::double precision>0 and char_length(fecha)>7 and char_length(hora2)>0)) then concat(fecha,' ',hora)::timestamp else nt.chartdate end as "CHARTTIME"
...
,"VALUE"
,"VALUEUOM"
from mioglobina_discharge_temp1
UNION
select
"SUBJECT_ID"
,"HADM_ID"
...
,"VALUE"
,"VALUEUOM"
from mioglobina_discharge_temp2
UNION
select
"SUBJECT_ID"
,"HADM_ID"
...
,"VALUE"
,"VALUEUOM"
from mioglobina_discharge_temp3
order by "SUBJECT_ID";
