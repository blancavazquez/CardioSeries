...
drop view NSTEMI_vent_class;
create view NSTEMI_vent_class as
select
...
\copy (SELECT * FROM NSTEMI_vent_class) to '/tmp/NSTEMI_vent_class.csv' CSV HEADER;

--***************************************
-- Paso2: added itemid, value and valueom for ventilation type, also
-- we did inner join with NSTEMI_icustays to add subject_id, hadm_id
...
drop view NSTEMI_vent_itemid;
create view NSTEMI_vent_itemid as
with temp_vent as
(
select 
   vt.icustay_id
...
from temp_vent vt
inner join NSTEMI_icustays icu
on vt.icustay_id = icu."ICUSTAY_ID"
where itemid IS NOT NULL 
order by vt.icustay_id;

\copy (SELECT * FROM NSTEMI_vent_itemid) to '/tmp/NSTEMI_VENTILATION.csv' CSV HEADER;
