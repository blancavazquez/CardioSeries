--- Script for extracting information of table of prescriptions for NSTEMI patients
...
drop view NSTEMI_prescriptions_preprocess cascade;
create view NSTEMI_prescriptions_preprocess as
  select
  pre.subject_id
  ,pre.hadm_id
  ,pre.icustay_id
...
drop view NSTEMI_prescriptions_join;
create view NSTEMI_prescriptions_join as
  select
  pre.subject_id
  ,pre.hadm_id
  ,pre.icustay_id
  ...
  from NSTEMI_prescriptions_preprocess pre
  order by pre.subject_id;

-- ******************************************************************
-- Paso 3: identificar tipo de unidad de medida por medicamento de interés
drop view NSTEMI_prescriptions_unit;
create view NSTEMI_prescriptions_unit as
  select
  pre.subject_id
  ,pre.drug
  ,pre.final_dosis
  ...
  from NSTEMI_prescriptions_join pre
...
drop view NSTEMI_prescriptions_itemid_dosis;
create view NSTEMI_prescriptions_itemid_dosis as
...
SELECT 
  ...
  ,pre.drug_name_generic
  ,pre.prod_strength
  ,REPLACE(pre.final_dosis,',','') as doses_administered ---- eliminar caracteres especiales de final_dosis
  ,pre.dose_unit_rx
  ,pre.intime
...
drop view NSTEMI_prescriptions_double;
create view NSTEMI_prescriptions_double as
  select
  ...
  from NSTEMI_prescriptions_itemid_dosis pre
  order by pre.subject_id;
\copy (SELECT * FROM NSTEMI_prescriptions_double) to '/tmp/NSTEMI_PRESCRIPTIONS.csv' CSV HEADER;
